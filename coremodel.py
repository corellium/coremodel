...
TYPE_STRING = ["uart", "i2c", "spi", "gpio", "usbh", "can", "eth", "event" ]
...
ETH_TX = ctypes.CFUNCTYPE(ctypes.c_int32, ctypes.c_void_p, ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint8))
ETH_RXRDY = ctypes.CFUNCTYPE(None, ctypes.c_void_p)

class coremodel_eth_func_t(ctypes.Structure):
    _fields_ = [
        ("tx", ETH_TX),
        ("rxrdy",ETH_RXRDY)
    ]

UPDATE = ctypes.CFUNCTYPE(None, ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint32)
ATRESP = ctypes.CFUNCTYPE(None, ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64)

class coremodel_event_func_t(ctypes.Structure):
    _fields_ = [
        ("update", UPDATE),
        ("atresp", ATRESP)
    ]
...
class CoreModel(threading.Thread):
    ...
    def __init__(self, name, address, port, path):
        ...
        self.libcm.coremodel_list_subdevices.argtypes = [ctypes.c_void_p, ctypes.c_int32, ctypes.c_char_p]
        self.libcm.coremodel_list_subdevices.restype = ctypes.c_void_p
        ...
        self.devlist = None
        self.devlist_evt =  None
        self.devlist_gpio = None
        self.devlist_spi = None
        ...
        self.libcm.coremodel_attach_gpio_name.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_char_p, ctypes.POINTER(coremodel_gpio_func_t), ctypes.c_void_p]
        self.libcm.coremodel_attach_gpio_name.restype = ctypes.c_void_p
        ...
        self.libcm.coremodel_attach_eth.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.POINTER(coremodel_eth_func_t), ctypes.c_void_p]
        self.libcm.coremodel_attach_eth.restype = ctypes.c_void_p

        self.libcm.coremodel_eth_rx.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint8)]
        self.libcm.coremodel_eth_rx.restype = ctypes.c_int32

        self.libcm.coremodel_attach_event_name.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.POINTER(coremodel_event_func_t), ctypes.c_void_p]
        self.libcm.coremodel_attach_event_name.restype = ctypes.c_void_p

        self.libcm.coremodel_event_signal.argtypes = [ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint32]
        self.libcm.coremodel_event_signal.restype = None

        self.libcm.coremodel_event_atomic.argtypes = [ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint32]
        self.libcm.coremodel_event_atomic.restype = None
    ...
    def _connect(self, address, port):
        ...
        while self.devlist[i].type != COREMODEL_INVALID:
            if self.devlist[i].type == COREMODEL_EVENT:
                c_type = ctypes.c_int(self.devlist[i].type)
                self.devlist_evt = self.libcm.coremodel_list_subdevices(self.cm, c_type.value, ctypes.c_char_p(self.devlist[i].name))
                if self.devlist_evt is not None:
                    self.devlist_evt = ctypes.cast(self.devlist_evt, ctypes.POINTER(coremodel_device_list_t))
            elif self.devlist[i].type == COREMODEL_GPIO:
                if self.devlist_gpio:
                    i += 1
                    continue
                c_type = ctypes.c_int(self.devlist[i].type)
                self.devlist_gpio = self.libcm.coremodel_list_subdevices(self.cm, self.devlist[i].type, ctypes.c_char_p(self.devlist[i].name))
                if self.devlist_gpio is not None:
                    self.devlist_gpio = ctypes.cast(self.devlist_gpio, ctypes.POINTER(coremodel_device_list_t))
            elif self.devlist[i].type == COREMODEL_SPI:
                c_type = ctypes.c_int(self.devlist[i].type)
                self.devlist_spi = self.libcm.coremodel_list_subdevices(self.cm, self.devlist[i].type, ctypes.c_char_p(self.devlist[i].name))
                if self.devlist_spi is not None:
                    self.devlist_spi = ctypes.cast(self.devlist_spi, ctypes.POINTER(coremodel_device_list_t))
            i += 1
    ...
    def device_list(self):
        ...
        while self.devlist[i].type != COREMODEL_INVALID:
            ...
            if self.devlist[i].type == COREMODEL_EVENT:
                if self.devlist_evt is not None:
                    j = 0
                    while self.devlist_evt[j].type != COREMODEL_INVALID:
                        if self.devlist_evt[j].name:
                            print("{:<16}{:<16}{:<16}{:<16}".format("", "", self.devlist_evt[j].name.decode('utf-8'), self.devlist_evt[j].num))
                        j += 1
            elif self.devlist[i].type == COREMODEL_GPIO:
                if self.devlist_gpio is not None:
                    print(self.devlist_gpio)
                    j = 0
                    while self.devlist_gpio[j].type != COREMODEL_INVALID:
                        if self.devlist_evt[j].name:
                            print("{:<16}{:<16}{:<16}{:<16}".format("", "", self.devlist_gpio[j].name.decode('utf-8'), self.devlist_gpio[j].num))
                        j += 1
            elif self.devlist[i].type == COREMODEL_SPI:
                if self.devlist_spi is not None:
                    j = 0
                    while self.devlist_spi[j].type != COREMODEL_INVALID:
                        if self.devlist_evt[j].name:
                            print("{:<16}{:<16}{:<16}{:<16}".format("", "", self.devlist_spi[j].name.decode('utf-8'), self.devlist_spi[j].num))
                        j += 1
            i += 1
    ...
    def _eth_rx(self, length, data):
        if self.handle is None:
            return 0
        c_len = ctypes.c_uint32(length)
        c_data = (ctypes.c_uint8 * len(data))(*data)
        ret = self._rx(self.handle, c_len.value, c_data)
        c_ret = ctypes.c_int32(ret)
        return c_ret.value

    def _evt_sig(self, data0, data1, change):
        c_data0 = ctypes.c_uint64(data0)
        c_data1 = ctypes.c_uint64(data1)
        c_change = ctypes.c_uint32(change)
        self._event_signal(self.handle, c_data0.value, c_data1.value, c_change.value)

    def _evt_atom(self, data0, data1, op):
        c_data0 = ctypes.c_uint64(data0)
        c_data1 = ctypes.c_uint64(data1)
        c_op = ctypes.c_uint32(op)
        self._event_atomic(self.handle, c_data0.value, c_data1.value, c_op.value)
    ...
    def attach(self, obj):
        ...
        while self.devlist[i].type != COREMODEL_INVALID:
            if self.devlist[i].name.decode('utf-8') == obj.name:
                if obj.type == COREMODEL_GPIO:
                    if self.devlist_gpio is not None:
                        j = 0
                        f = 0
                        if type(obj.pin) is int:
                            break
                        while self.devlist_gpio[j].type != COREMODEL_INVALID:
                            if self.devlist_evt[j].name:
                                if self.devlist_gpio[j].name.decode('utf-8') == obj.pin:
                                    f = 1
                                    break
                            j += 1
                        if f:
                    ...
                    break
            elif obj.type == COREMODEL_EVENT:
                if self.devlist_evt is not None:
                    j = 0
                    f = 0
                    while self.devlist_evt[j].type != COREMODEL_INVALID:
                        if self.devlist_evt[j].name:
                            if self.devlist_evt[j].name.decode('utf-8') == obj.name:
                                f = 1
                                break
                        j += 1
                    if f:
        ...
        if obj.type == COREMODEL_UART:
            ...
            elif type(obj.pin) is str:
                obj.handle = self.libcm.coremodel_attach_gpio_name(self.cm, obj.name.encode('utf-8'), obj.pin.encode('utf-8'), ctypes.pointer(obj.gpio_func), priv)
        ...
        elif obj.type == COREMODEL_ETH:
            print("Attaching Ethernet")
            obj.eth_func = coremodel_eth_func_t(tx = ETH_TX(obj._tx), rxrdy = ETH_RXRDY(obj._rxrdy))
            obj.handle = self.libcm.coremodel_attach_eth(self.cm, obj.name.encode('utf-8'), ctypes.pointer(obj.eth_func), priv)
            if obj.handle is None:
                raise NameError("Attach Ethernet Failed")
            else:
                obj._rx = self.libcm.coremodel_eth_rx
                obj.rx = MethodType(CoreModel._eth_rx, obj)
                self.attached_objs.append(obj)
                obj.cm = self

        elif obj.type == COREMODEL_EVENT:
            print("Attaching Event")
            obj.event_func = coremodel_event_func_t(update = UPDATE(obj._update), atresp = ATRESP(obj._atresp))
            obj.handle = self.libcm.coremodel_attach_event_name(self.cm, obj.name.encode('utf-8'), ctypes.pointer(obj.event_func), priv)
            if obj.handle is None:
                raise NameError("Attach Event Failed")
            else:
                obj._event_signal = self.libcm.coremodel_event_signal
                obj.event_signal = MethodType(CoreModel._evt_sig, obj)
                obj._event_atomic = self.libcm.coremodel_event_atomic
                obj.event_atomic = MethodType(CoreModel._evt_atom, obj)
                self.attached_objs.append(obj)
                obj.cm = self
...
class CoreModelEth():
    def __init__(self, busname):
        self.name = busname
        self.type = COREMODEL_ETH
        self.handle = None
        self.cm = None

    @staticmethod
    def _tx(obj, length, data):
        py_obj = ctypes.cast(obj, ctypes.py_object).value
        c_length = ctypes.c_uint32(length)
        c_data = (ctypes.c_uint8 * c_length.value).from_address(ctypes.addressof(data.contents))
        ret = py_obj.tx(c_length.value, c_data)
        c_ret = ctypes.c_int32(ret)
        return c_ret.value
...
class CoreModelEvent():
    def __init__(self, eventname):
        self.name = eventname
        self.type = COREMODEL_EVENT
        self.handle = None
        self.cm = None

    @staticmethod
    def _update(obj, data0, data1, initial):
        py_obj = ctypes.cast(obj, ctypes.py_object).value
        c_data0 = ctypes.c_uint64(data0)
        c_data1 = ctypes.c_uint64(data1)
        c_initial = ctypes.c_int32(initial)
        py_obj.update(c_data0.value, c_data1.value, c_initial.value)

    def update(self, data0, data1, initial):
        pass

    @staticmethod
    def _atresp(obj, data0, data1, op):
        py_obj = ctypes.cast(obj, ctypes.py_object).value
        c_data0 = ctypes.c_uint64(data0)
        c_data1 = ctypes.c_uint64(data1)
        c_op = ctypes.c_uint32(op)
        py_obj.atresp(c_data0.value, c_data1.value, c_op.value)

    def atresp(self, data0, data1, op):
        pass

    def _event_signal(self):
        pass #defined by coremodel

    def event_signal(self):
        pass #defined by coremodel

    def _event_atomic(self):
        pass #defined by coremodel

    def event_atomic(self):
        pass #defined by coremodel